...
## Transparent order-flow venues

- Hyperliquid など透明な order flow を持つ venue では、短期情報量が market maker に有利に働く一方、大口/遅い参加者には adverse selection が増える。
- 透明性を「流動性が良い」とだけ評価しない。flow visibility、front-run/MEV risk、latency、fee、privacy、custody/security を同時に見る。
- 大口執行では、公開 flow と秘匿化/ミキシング/OTC/分割発注のトレードオフを明示する。秘匿化サービスは単一口座集約や custody risk を別途レビューする。
- DEX/CEX比較では、手数料だけでなく「どの情報が誰にいつ見えるか」を Execution Model に含める。
