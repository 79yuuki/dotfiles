...
## Capital management add-ons

- 資金管理を「損切り幅」だけに縮小しない。トレード単位の sizing とポートフォリオ単位の risk budget を分けて設計する。
- 予測精度が同じでも、sizing、相関、執行制約で資産曲線は別物になる。勝率/精度より、破綻しないサイズとドローダウン制御を優先する。
- ATR / realized volatility によるサイズ調整は、ボラ急変時にサイズが過大化しないよう上限・遅延・クールダウンを持つ。
- 複数戦略を走らせる時は、見かけ上の分散ではなく、共通リスク（市場急変、流動性、取引所/API、同一ファクター）への集中を測る。
- AI提案による増額は、独立レビューと `APPROVED_WITH_LIMITS` の条件がない限り禁止する。
